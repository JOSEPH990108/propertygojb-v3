        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="space-y-6">
            <section className="border border-slate-200 bg-white shadow-sm">
              ...
              <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-bold text-slate-600">Each coloured cell shows its unit number and live availability. Grey cells are core or service positions; dashed cells are non-saleable on that floor. Select a unit to see its type, view, and facing.</p><label className="relative block w-full sm:w-44"><span className="sr-only">Select floor</span><select value={activeFloor} onChange={(event) => { setSelectedFloor(Number(event.target.value)); setSelectedUnitId(null); }} className="h-10 w-full appearance-none border border-slate-200 bg-white px-3 pr-9 text-sm font-black text-slate-950 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100">{floors.map((floor) => <option key={floor} value={floor}>Floor {floor}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3 size-4 text-slate-500" /></label></div>
              <div className="overflow-auto py-5">
                <div className="min-w-[55rem]" style={{ minWidth: `${Math.max(55, physicalStacks.length * 3.2 + 5)}rem` }}>
                  <div className="grid border border-slate-200" style={{ gridTemplateColumns: focusColumns }}>
                    <div className="flex items-center justify-center border-b border-r border-slate-200 bg-slate-100 px-3 text-xs font-black text-slate-600">View</div>
          ...
          <aside className="border border-slate-200 bg-white p-6 shadow-sm xl:sticky xl:top-6 xl:self-start">{selectedUnit ? <div><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Selected residence</p><h2 className="mt-2 text-2xl font-black text-slate-950">{selectedUnit.unitNo}</h2></div><button type="button" onClick={() => setSelectedUnitId(null)} className="grid size-9 place-items-center border border-slate-200 text-slate-500 transition hover:bg-slate-50" aria-label="Close selected residence"><X className="size-4" /></button></div><span className={`mt-5 inline-flex items-center gap-2 border px-3 py-1.5 text-xs font-black ${statusStyles[getUnitStatus(selectedUnit.bookingStatusCode)].cell}`}><span className={`size-2 rounded-full ${statusStyles[getUnitStatus(selectedUnit.bookingStatusCode)].swatch}`} />{selectedUnit.bookingStatusName ?? statusStyles[getUnitStatus(selectedUnit.bookingStatusCode)].label}</span><div className="mt-8 space-y-5 border-y border-slate-100 py-5 text-sm"><div className="flex items-center justify-between"><span className="text-slate-500">Type</span><span className="font-black text-slate-950">{selectedLayout?.code ?? "-"}</span></div><div className="flex items-center justify-between"><span className="text-slate-500">View</span><span className="inline-flex items-center gap-1 font-black text-slate-950"><Compass className="size-3 text-blue-600" />{getViewLabel(selectedUnit)}</span></div><div className="flex items-center justify-between"><span className="text-slate-500">Facing</span><span className="font-black text-slate-950">{formatFacing(selectedUnit.facing)}</span></div><div className="flex items-center justify-between"><span className="text-slate-500">Floor / stack</span><span className="font-black text-slate-950">{selectedUnit.floor} / {selectedUnit.stack}</span></div><div className="flex items-center justify-between"><span className="text-slate-500">Built-up</span><span className="font-black text-slate-950">{selectedUnit.builtUpSqft ? `${selectedUnit.builtUpSqft} sqft` : "-"}</span></div></div><p className="mt-7 text-xs font-black uppercase tracking-[0.18em] text-slate-400">Current price</p><p className="mt-2 flex items-center gap-2 text-2xl font-black text-slate-950"><CircleDollarSign className="size-5 text-emerald-600" />{formatMoney(selectedUnit.finalPrice ?? selectedUnit.basePrice)}</p></div> : <div className="flex min-h-72 flex-col justify-end border-l-4 border-blue-600 pl-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Residence details</p><h2 className="mt-3 text-2xl font-black tracking-tight text-slate-950">Select a unit</h2><p className="mt-3 max-w-xs text-sm leading-6 text-slate-600">Choose Tower B or C, then select a unit to inspect its number, type, facing and price.</p><div className="mt-8 flex items-center gap-2 text-sm font-bold text-slate-500"><Ruler className="size-4 text-blue-600" />Tower {activeTower} has {actualStacks.length} sellable stacks</div></div>}</aside>
        </div>
      </div>
    </div>
  );
