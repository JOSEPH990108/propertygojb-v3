...
type BookingNoteComposerProps = {
  bookingId: string;
};

type BookingNoteResult = {
  bookingId: string;
  message?: string;
};

export function BookingNoteComposer({ bookingId }: BookingNoteComposerProps) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await postJson<BookingNoteResult>(
        "/api/internal/bookings/note",
        {
          bookingId,
          note,
        },
      ...
      appToast.success(result.message ?? "Booking note added.");
      setNote("");
      router.refresh();
    });
  }
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            Save internal booking notes for follow-up, payment checking,
            document checking, or approval context.
          </p>
        </div>
      </div>
      ...
      <form
        ...
        className="mt-5 grid gap-4 xl:grid-cols-[1fr_auto] xl:items-start"
      >
        <div className="space-y-2">
          ...
          <textarea
            ...
            disabled={isPending}
            rows={4}
            placeholder="Example: Customer confirmed booking fee payment, pending IC copy..."
